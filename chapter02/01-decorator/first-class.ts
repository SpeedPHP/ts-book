...
function atClassWithArgs(...args: any[]) {
    return function (target: any) {
        console.log("类装饰器有参数，参数值为", args.join(","));
    }
}
...
function atMethod(target: any, propertyKey: string) {
    ...
    console.log("方法装饰器，获得返回类型为", returnType.name);
}

// 方法装饰器，带参数
function atMethodWithArgs(...args: any[]) {
    return function (target: any, propertyKey: string) {
        console.log("方法装饰器有参数，参数值为", args.join(","));
    }
}
...
function atPropertyWithArgs(...args: any[]) {
    return function (target: any, propertyKey: string) {
        console.log("变量装饰器有参数，参数值为", args.join(","));
...
function atAccessor(target: any, propertyKey: string, descriptor: PropertyDescriptor) {
    ...
    console.log("访问器装饰器，访问器类型是", returnType, "，其值为", target[propertyKey]);
}

// 访问器装饰器，带参数
function atAccessorWithArgs(...args: any[]) {
    return function (target: any, propertyKey: string, descriptor: PropertyDescriptor) {
        console.log("访问器装饰器，参数值为", args.join(","));
    }
}
...
function atParameter(target: any, propertyKey: string, parameterIndex: number) {
    ...
    console.log("参数装饰器，参数位置在", parameterIndex, "，其参数类型是", parameterType[parameterIndex].name);
}

// 参数装饰器，带参数
function atParameterWithArgs(...args: any[]) {
    return function (target: any, propertyKey: string, parameterIndex: number) {
        console.log("参数装饰器有参数，参数值为", args.join(","));
    }
}
